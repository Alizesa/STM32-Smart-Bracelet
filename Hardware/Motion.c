...
/**
  * @brief 跌倒状态机: 自由落体 -> 冲击 -> 静止
  * @param  mag     当前合成加速度 (LSB)
  * @param  now_ms  当前时间 ms
  */
static void Fall_Update(float mag, uint32_t now_ms)
{
	static uint8_t  armed = 0;          /* 已确认自由落体, 等待冲击 */
	static uint32_t armTime = 0;
	static uint8_t  postImpact = 0;     /* 已发生冲击, 观察是否静止 */
	static uint32_t postImpactTime = 0;
	static int32_t  maxDev = 0;         /* 观察窗口内最大偏离量 */

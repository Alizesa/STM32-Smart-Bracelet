...
void MPU6050_EnableInt(uint8_t motionEn, uint8_t freeFallEn)
{
	...
	if (freeFallEn)
	{
		MPU6050_WriteReg(MPU6050_FF_THR, 0x0A);		//自由落体阈值 ~20mg
		MPU6050_WriteReg(MPU6050_FF_DUR, 0x14);		//持续 20ms
		enable |= 0x40;								//FF_EN
	}
	if (motionEn)
	{
	...
	MPU6050_WriteReg(MPU6050_INT_ENABLE, enable);
}
...
{
	return MPU6050_ReadReg(MPU6050_INT_STATUS);
}
